...
def service(detection_settings: Settings, tmp_path: Path) -> DetectionService:
    ...
    with patch("backend.detection.service.get_settings", return_value=detection_settings), \
...
class TestDetectionServiceInit:
    ...
    def test_current_model_id_none_before_training(
        self, service: DetectionService
    ) -> None:
        assert service.current_model_id is None

    def test_score_event_before_training_raises(
        self, service: DetectionService
    ) -> None:
    ...
    def test_score_batch_before_training_raises(
        self, service: DetectionService
    ) -> None:
        with pytest.raises(ModelNotTrainedError):
            service.score_batch_from_features(feature_records=[make_feature_record()])

    def test_stream_before_training_raises(
        self, service: DetectionService
    ) -> None:
        with pytest.raises(ModelNotTrainedError):
...
class TestDetectionServiceTraining:
    ...
    def test_model_loaded_after_training(
        self, service: DetectionService
    ) -> None:
    ...
    def test_model_id_set_after_training(
        self, service: DetectionService
    ) -> None:
        records = make_normal_records(50)
        result = service.train_from_features(feature_records=records)
    ...
    def test_model_paths_set_in_result(
        self, service: DetectionService
    ) -> None:
        records = make_normal_records(50)
        result = service.train_from_features(feature_records=records)
    ...
    def test_incremental_retrain_increases_model_count(
        ...
        result = service.retrain_incremental(
            new_records, existing_records=initial
        )
        assert result.sample_count == 50 + 20

    def test_retrain_uses_passed_existing_records(
        self, service: DetectionService
    ) -> None:
        records = make_normal_records(50)
        service.train_from_features(feature_records=records)
    ...
    def test_get_status_after_training(
        self, trained_service: DetectionService
    ) -> None:
    ...
    def test_get_status_before_training(
        self, service: DetectionService
    ) -> None:
...
class TestDetectionServiceInference:
    def test_score_event_returns_none_or_alert(
        self, trained_service: DetectionService
    ) -> None:
    ...
    def test_score_event_alert_has_model_id(
        self, trained_service: DetectionService
    ) -> None:
        records = make_normal_records(50)
        for rec in records:
    ...
    def test_score_batch_returns_detection_result(
        self, trained_service: DetectionService
    ) -> None:
    ...
    def test_score_stream_yields_alerts(
        self, trained_service: DetectionService
    ) -> None:
    ...
    def test_reload_model_after_training(
        self, trained_service: DetectionService
    ) -> None:
    ...
    def test_list_available_models_after_two_trains(
        self, service: DetectionService
    ) -> None:
...
class TestDetectionServiceFeatureLoading:
    def test_load_from_jsonl_files(
        self, service: DetectionService, tmp_path: Path
    ) -> None:
        features_dir = tmp_path / "data" / "features"
